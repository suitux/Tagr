...
- `src/infrastructure/` — Database utilities: `dbClient.ts` (Prisma singleton), `metadata-scan.service.ts` (file traversal + metadata extraction), `metadata-write.service.ts` (writes tags back to files), `optimize.ts` (SQLite PRAGMAs: WAL, 256MB cache)
