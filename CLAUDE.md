...
- `src/infrastructure/` — Database utilities: `dbClient.ts` (Prisma singleton), `scanner.ts` (file traversal + metadata extraction), `metadata-writer.ts` (writes tags back to files), `optimize.ts` (SQLite PRAGMAs: WAL, 256MB cache)
