...
async function fetchPlaylistSongs(
  ...
  const { data } = await api.get<SongsResponse>(`/playlists/${playlistId}/songs`, { params })
  return data
}

export const getPlaylistSongsQueryKey = (
  playlistId: number | null,
  search?: string,
  sorting?: SongsSortParams,
  filters?: SongColumnFilters,
  metadataKeys?: string[]
) => ['playlists', playlistId, 'songs', search, sorting?.sortField, sorting?.sort, filters, metadataKeys]

interface UsePlaylistSongsParams {
  playlistId: number | null
  search?: string
  sorting?: SongsSortParams
  filters?: SongColumnFilters
...
export function usePlaylistSongs({ playlistId, search, sorting, filters, metadataKeys }: UsePlaylistSongsParams) {
  return useInfiniteQuery({
    queryKey: getPlaylistSongsQueryKey(playlistId, search, sorting, filters, metadataKeys),
    queryFn: ({ pageParam = 0 }) =>
